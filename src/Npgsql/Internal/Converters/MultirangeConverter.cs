    ...
    where TRange : notnull
{
    ...
    protected override Size GetSize(SizeContext context, T value, ref object? writeState)
    {
        ...
        for (var i = 0; i < value.Count; i++)
        {
            ...
            var rangeSize = _rangeConverter.IsDbNull(value[i], innerState) ? null : (Size?)_rangeConverter.Bind(new(context.Format, _rangeRequirements.Write), value[i], ref innerState);
