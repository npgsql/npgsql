...
class UnmappedRangeTypeInfoResolver : DynamicTypeInfoResolver
{
    protected override DynamicMappingCollection? GetMappings(Type? type, DataTypeName dataTypeName, PgSerializerOptions options)
    {
        ...
        if (type is not null && !IsTypeOrNullableOfType(type, static type => type.IsConstructedGenericType && type.GetGenericTypeDefinition() == typeof(NpgsqlRange<>), out matchedType)
        ...
        // We have no generic RangeConverterResolver so we would not know how to compose a range mapping for such subtype infos.
