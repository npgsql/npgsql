...
sealed class ConnectorTypeMapper : TypeMapperBase
{
    ...
    internal bool TryResolveByOID(uint oid, [NotNullWhen(true)] out NpgsqlTypeHandler? handler)
    {
        ...
        lock (_writeLock)
        {
            if ((handler = ResolveByDataTypeName(pgType.DisplayName, throwOnError: false)) is not null)
            {
    ...
    {
        foreach (var resolver in _resolvers)
            if ((mapping = resolver.GetMappingByDataTypeName(pgType.DisplayName)) is not null)
                return true;
    ...
    internal (NpgsqlDbType? npgsqlDbType, PostgresType postgresType) GetTypeInfoByOid(uint oid)
    {
        ...
        foreach (var resolver in _resolvers)
            if (resolver.GetMappingByDataTypeName(pgType.DisplayName) is { } mapping)
