...
public class ReaderNewSchemaTests : SyncOrAsyncTestBase
{
    ...
    public async Task IsAutoIncrement()
    {
        ...
        await using var _ = await CreateTempTable(conn, "serial SERIAL, int INT", out var table);
    ...
    public async Task IsAutoIncrement_identity()
    {
        ...
        await using var _ = await CreateTempTable(
            conn,
            "identity1 INT GENERATED BY DEFAULT AS IDENTITY, identity2 INT GENERATED ALWAYS AS IDENTITY",
            out var table);
...
CREATE TABLE {table2} (foo INTEGER)");
    ...
    public async Task Domain_type()
    {
        ...
        await using var _ = await GetTempTypeName(conn, out var domainTypeName, "pg_temp");
