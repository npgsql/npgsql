...
public class ReaderNewSchemaTests(SyncOrAsync syncOrAsync) : SyncOrAsyncTestBase(syncOrAsync)
{
    ...
    public async Task Partitioned_table()
    {
        await using var conn = await OpenConnectionAsync();
        await IgnoreOnRedshift(conn);
        MinimumPgVersion(conn, "10.0", "Partitioned tables introduced in PostgreSQL 10");
        ...
        await conn.ExecuteNonQueryAsync(
            $"""
             CREATE TABLE {table} (
                 ...
                 CONSTRAINT pk_{table} PRIMARY KEY (id, event_date)
             ) PARTITION BY RANGE (event_date)
             """);

        await using var cmd = new NpgsqlCommand($"SELECT id, event_date, user_name FROM {table}", conn);
        await using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
        var columns = await GetColumnSchema(reader);

        Assert.That(columns[0].AllowDBNull, Is.False);
        Assert.That(columns[0].IsAutoIncrement, Is.True);
        Assert.That(columns[1].AllowDBNull, Is.False);
        Assert.That(columns[1].IsAutoIncrement, Is.False);
        Assert.That(columns[2].AllowDBNull, Is.True);
        Assert.That(columns[2].IsAutoIncrement, Is.False);
    }
