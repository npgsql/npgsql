...
static class DefaultPgTypes
{
    ...
    public static IEnumerable<PgTypeGroup> Items
    {
        get
        {
            ...
            Array.Sort(items, (first, second) => first.TypeKind.CompareTo(second.TypeKind));
            return items;
        }
    }
}
...
readonly struct PgTypeGroup
{
    ...
    public Oid? SubTypeOid { get; init; }
    ...
    public static PgTypeGroup Create(DataTypeName name, uint oid, uint arrayOid, uint? subTypeOid, string? multirangeName = null, uint? multirangeOid = null, uint? multirangeArrayOid = null, PgTypeKind typeKind = PgTypeKind.Base)
    {
        ...
        {
            if (subTypeOid is null)
                throw new ArgumentException("When a range is supplied its subtype oid cannot be omitted.");
        ...
        {
            if (subTypeOid is not null)
                throw new ArgumentException("Only range types can have a subtype oid.");
