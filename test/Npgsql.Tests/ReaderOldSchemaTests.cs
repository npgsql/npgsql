...
CREATE TABLE {table} (
    ...
    CONSTRAINT {table}_pk PRIMARY KEY (field_pk1, field_pk2)
        ...
        using var command = new NpgsqlCommandOrig($"SELECT * FROM {table}", conn);
    ...
    public async Task IsAutoIncrement()
    {
        ...
        var command = new NpgsqlCommandOrig($"SELECT serial, int, 8 FROM {table}", conn);
    ...
    public async Task IsAutoIncrement_identity()
    {
        ...
        await using var command = new NpgsqlCommandOrig($"SELECT identity1, identity2 FROM {table}", conn);
        await using var reader = command.ExecuteReader(CommandBehavior.KeyInfo);
        var rows = (await GetSchemaTable(reader))!.Rows;
...
CREATE OR REPLACE VIEW {view} (id, int2) AS SELECT id, int2 + int2 AS int2 FROM {table}");
        ...
        var command = new NpgsqlCommandOrig($"SELECT * FROM {view}", conn);
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var cmd = new NpgsqlCommandOrig("SELECT 1::NUMERIC AS result", conn);
        using var reader = await cmd.ExecuteReaderAsync();
...
SELECT 1 AS some_other_column, 2";
    ...
    public async Task BaseColumnName()
    {
        ...
        var cmd = new NpgsqlCommandOrig("SELECT Cod as CodAlias, Descr as DescrAlias, Date FROM data", conn);
