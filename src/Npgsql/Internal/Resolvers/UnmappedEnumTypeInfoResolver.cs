...
class UnmappedEnumTypeInfoResolver : DynamicTypeInfoResolver
{
    ...
    {
        if (type is null || !IsTypeOrNullableOfType(type, static type => type.IsEnum, out var matchedType) || options.TypeCatalog.GetPostgresTypeByName(dataTypeName) is not PostgresEnumType)
            return null;
