...
{
    partial class TimestampTzHandler
    {
        public override int ValidateObjectAndGetLength(object value, ref NpgsqlLengthCache? lengthCache, NpgsqlParameter? parameter)
            => value switch
            {
                ...
                NpgsqlDateTime converted => ((INpgsqlSimpleTypeHandler<NpgsqlDateTime>)this).ValidateAndGetLength(converted, parameter),
                
                DateTimeOffset converted => ((INpgsqlSimpleTypeHandler<DateTimeOffset>)this).ValidateAndGetLength(converted, parameter),
                

                _ => throw new InvalidCastException($"Can't write CLR type {value.GetType()} with handler type TimestampTzHandler")
            };
        ...
        public override Task WriteObjectWithLength(object? value, NpgsqlWriteBuffer buf, NpgsqlLengthCache? lengthCache, NpgsqlParameter? parameter, bool async, CancellationToken cancellationToken = default)
            => value switch
            {
                ...
                NpgsqlDateTime converted => WriteWithLength(converted, buf, lengthCache, parameter, async, cancellationToken),
