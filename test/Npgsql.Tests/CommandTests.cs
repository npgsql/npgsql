...
namespace Npgsql.Tests
{
    public class CommandTests : TestBase
    {
        ...
        public void PrepareMultipleCommandsWithParameters()
        {
            ...
            var p1 = new NpgsqlParameter("p1", NpgsqlDbType.Integer) { Value = 8 };
            var p21 = new NpgsqlParameter("p1", NpgsqlDbType.Text) { Value = "foo" };
            var p22 = new NpgsqlParameter("p2", NpgsqlDbType.Text) { Value = "bar" };
        ...
        public void CursorStatement()
        {
            ...
            var i = 0;
            using var command = new NpgsqlCommand("DECLARE TE CURSOR FOR SELECT * FROM DATA", conn);
        ...
        public void CursorMoveRecordsAffected()
        {
            ...
            using var command = new NpgsqlCommand("DECLARE curs CURSOR FOR SELECT * FROM (VALUES (1), (2), (3)) as t", connection);
            command.ExecuteNonQuery();
        ...
        public void SameParamMultipleTimes()
        {
            ...
            using var reader = cmd.ExecuteFirstRecord();
        ...
        {
            using var conn = OpenConnection();
            using var command = new NpgsqlCommand("select 3, 4 as param1, 5 as param2, 6;", conn);
...
@"CREATE OR REPLACE FUNCTION pg_temp.more_params(OUT a integer, OUT b boolean) AS
            ...
            conn.ExecuteNonQuery(createFunction);

            using var command = new NpgsqlCommand("pg_temp.more_params", conn)
            {
